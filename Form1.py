...
n = 8            # Example: 8 plants
T = 168          # Example: 168 time periods
...
#     a. -o[t,i] <= u[t-1,i] - u[t,i]: Ensures o[t,i] captures the startup (transition from off to on)
for i in plants:
    t = 1
    model.addConstr(-o[t,i] <= u0[i] - u[t,i],
                        name=f"StartupLogic1_t{t}_p{i}")
    for t in range(2, T+1):
        model.addConstr(-o[t,i] <= u[t-1,i] - u[t,i],
                        name=f"StartupLogic1_t{t}_p{i}")
...
for i in plants:
    t = 1
    model.addConstr(o[t,i] <= 1 - u0[i],
                        name=f"StartupLogic3_t{t}_p{i}")
    for t in range(2, T+1):
        model.addConstr(o[t,i] <= 1 - u[t-1,i],
                        name=f"StartupLogic3_t{t}_p{i}")
